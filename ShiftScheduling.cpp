...
/* ShiftScheduling takes in a set of shifts along with their times, values, and the
 * amount of hours a worker can work. Then, it returns the best (most valuable)
 * combination of shifts for the worker.
 */

/* Checks if new set is better than the current best set of shifts */
void checkBest(Set<Shift> chosenShifts, Set<Shift>& bestShifts, int& valBest) {
    int val = 0;
    for (Shift time : chosenShifts) {
        val += valueOf(time);
    }
    if (val > valBest) {
        valBest = val;
        bestShifts = chosenShifts;
    }
}

/* Checks if the time of the given shift overlaps with any of the already chosen shifts */
bool checkIfNotOverlapping(const Set<Shift>& chosenShifts, const Shift& shift) {
    for (Shift chosen: chosenShifts) {
        if (overlapsWith (chosen, shift)) {
            return false;
        }
    }
    return true;
...
/* Creates all the feasible combinations of shifts and updates bestShifts to the most
 * valuable combination. */
void highestValueScheduleForRec(const Set<Shift>& shifts, int maxHours, Set<Shift> chosenShifts,
                                      Set<Shift> remaining, Set<Shift>& bestShifts, int& valBest) {

    /* If no more shifts to choose from or length of shift is greater than hours left, don't choose more
     * Instead, now check if it's the best shift so far (and replace if so)*/
    if (remaining.isEmpty() || lengthOf(remaining.first()) > maxHours) {
        checkBest(chosenShifts, bestShifts, valBest);
    }
    else {
        Shift shift = remaining.first();

        /* Without this shift */
        highestValueScheduleForRec(shifts, maxHours, chosenShifts, remaining - shift, bestShifts, valBest);

        /* Check if can include this shift*/
        if (checkIfNotOverlapping(chosenShifts, shift)) {
            /* With this shift */
            highestValueScheduleForRec(shifts, maxHours - lengthOf(shift), chosenShifts + shift,
                                       remaining - shift, bestShifts, valBest);
        }
    }
}
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    if (maxHours < 0) {
        error("Hours allowed is less than 0");
    }
    Set<Shift> chosenShifts = {};
    Set<Shift> remaining = shifts;
    Set<Shift> bestShifts;
    int valBest = 0;
    highestValueScheduleForRec(shifts, maxHours, chosenShifts, remaining, bestShifts, valBest);
    return bestShifts;
}
...
/*Add test where 2 sets of shifts have the same value*/
